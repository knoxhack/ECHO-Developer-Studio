export type UpdateFeedConfig = { provider: 'github'; owner: string; repo: string; releaseType: 'release' }
...
export function selectIndexedProductUpdate(entry: ReleaseIndexProductEntry, productId: string): IndexedProductUpdate {
  ...
  return {
    entry,
    feed: { provider: 'github', owner: match[1], repo: match[2], releaseType: 'release' },
    artifacts: {
      latestYml,
      installer,
      ...(blockmap ? { blockmap } : {}),
